...
#define HL_RX_BUFFER_SIZE 2048 // Can be larger if desired
...
int uart_receive(uint8_t* buf, uint16_t size){
	int delay = 1000;
	while(uartdata.offset<size && delay>0){
		HAL_Delay(1);
		delay--;
	}
	if (delay==0){
	...
	memcpy(buf,&(uartdata.data[uartdata.offset]), size);
	memmove(uartdata.data, &(uartdata.data[uartdata.offset]), uartdata.offset-size);
	uartdata.offset -= size;
	return 0;
}

void uart_transmit_ch(uint8_t ch){
	CDC_Transmit_FS(&ch,1);
}

void uart_transmit_str(uint8_t* str){
	CDC_Transmit_FS(str,(uint16_t)strlen((char*)str));
}
